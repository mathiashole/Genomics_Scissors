...
sub extractor {
    ...
    foreach my $coord (@coordinates) {
        ...
        if (exists $hash_sequence{$name}) {
            ...
            my $sequence = extract_sequence($contig, $start, $end, $rest[0]);  # Assuming the strand info is in $rest[0]
...
sub extract_sequence {
    ...
    # Set coordinates for indexing
    $start_coordinate--;
    $end_coordinate--;
    
    # Calculate the length of the extracted sequence
    my $length = abs($end_coordinate - $start_coordinate) + 1;
    my $result = substr($sequence, $start_coordinate, $length);
    
    # Determines orientation based on coordinates if $strand is not defined
    if (defined $strand) {
        if ($strand eq '-') {
            $result = reverse_complement($result);
        }
    } else {
        # If the start is greater than the end, we assume reverse orientation
